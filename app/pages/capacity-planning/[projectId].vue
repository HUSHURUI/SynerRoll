...
const updateParticipation = (item: CapacityVariableDraft, event: Event) => {
  const participates = (event.target as HTMLInputElement).checked
      ...
      <template v-else-if="schema">
        <aside class="panel-card flex w-72 shrink-0 flex-col overflow-hidden">
          <div class="border-b border-app-border px-5 py-3">
            ...
            <p class="mt-1 text-xs leading-5 text-app-muted">按步骤完成配置、场景生成、求解和方案应用。</p>
          </div>
          ...
          <nav class="flex-1 overflow-y-auto px-3 py-3" aria-label="容量规划步骤">
            <template v-for="step in planningSteps" :key="step.id">
              ...
              >
                <div class="flex h-full items-center gap-3">
                  ...
                  <span class="min-w-0 flex-1">
                    <span class="block truncate text-sm font-semibold">步骤{{ ['一', '二', '三', '四', '五', '六'][step.id - 1] }}：{{ step.title }}</span>
                    <span
                      class="mt-1 block truncate text-xs leading-5"
                      :class="currentStep === step.id ? 'text-blue-100' : 'text-app-muted'"
                    >
                      {{ step.description }}
                    </span>
                  </span>
              ...
              <div v-if="step.id < 6" class="flex h-7 items-center justify-center text-2xl font-black leading-none text-primary/45">
                ↓
              </div>
            </template>
          </nav>

          <div class="border-t border-app-border bg-app-panel-soft px-4 py-3 text-xs text-app-muted">
            当前：<span class="font-semibold text-primary">{{ activeStep.title }}</span>
          </div>
        </aside>
        ...
        <main class="panel-card flex min-w-0 flex-1 flex-col overflow-hidden">
          <div class="flex items-center justify-between gap-5 border-b border-app-border bg-white px-6 py-4">
            <div>
              ...
              <h2 class="mt-0.5 text-xl font-bold text-app-text">{{ activeStep.title }}  -  {{ activeStep.description }}</h2>
            </div>
          </div>
          ...
          <div ref="contentScroller" class="min-h-0 flex-1 overflow-y-auto bg-app-surface p-5">
            ...
            <section v-if="currentStep === 1" class="space-y-4">
              ...
              <div class="overflow-hidden rounded-lg border border-app-border bg-white">
                <div class="flex items-center justify-between border-b border-app-border px-5 py-4">
                  <div>
                    <h3 class="font-semibold text-app-text">容量参数输入</h3>
                    <p class="mt-1 text-xs text-app-muted">勾选参与容量优化的设备，输入容量上下界，并拖动三角标记设置推荐值。</p>
                  </div>
                  <span class="hidden text-xs text-app-muted sm:inline">上下界可输入</span>
                </div>

                <div v-if="variables.length" class="space-y-3 p-4">
                  <article
                    v-for="item in variables"
                    :key="item.componentId"
                    class="grid grid-cols-[minmax(160px,220px)_150px_140px] justify-center gap-x-5 gap-y-3 rounded-lg border p-3 transition-all xl:grid-cols-[180px_150px_140px_minmax(380px,460px)] xl:items-center"
                    :class="item.mode === 'optimize'
                      ? 'border-primary/35 bg-primary-soft/45 shadow-sm'
                      : 'border-app-border bg-app-panel-soft text-app-muted'"
                  >
                    <div class="min-w-0 self-center">
                      <div class="truncate text-sm font-semibold" :class="item.mode === 'optimize' ? 'text-app-text' : 'text-app-muted'">
                        {{ item.componentName }}
                      </div>
                      <div class="mt-0.5 truncate text-xs text-app-muted">{{ item.componentKey }}</div>
                    </div>

                    <div class="w-fit justify-self-center text-left">
                      <div class="text-[11px] font-medium" :class="item.mode === 'optimize' ? 'text-primary' : 'text-app-muted'">当前容量</div>
                      <div class="mt-0.5 text-lg font-bold leading-none tracking-tight" :class="item.mode === 'optimize' ? 'text-primary' : 'text-app-muted'">
                        {{ formatNumber(item.currentValue) }}
                        <span class="text-xs font-semibold">{{ item.unit }}</span>
                      </div>
                    </div>

                    <label
                      class="inline-flex h-9 cursor-pointer items-center justify-center justify-self-center gap-2 whitespace-nowrap rounded-md border px-3 text-xs font-semibold transition"
                      :class="item.mode === 'optimize'
                        ? 'border-primary/30 bg-white text-primary shadow-sm'
                        : 'border-app-border bg-white/60 text-app-muted'"
                    >
                      <input
                        type="checkbox"
                        class="h-4 w-4 cursor-pointer accent-primary"
                        :checked="item.mode === 'optimize'"
                        @change="updateParticipation(item, $event)"
                      >
                      <span>{{ item.mode === 'optimize' ? '参与容量优化' : '不参与容量优化' }}</span>
                    </label>

                    <div v-if="item.mode === 'optimize'" class="col-span-3 w-full max-w-[520px] min-w-0 justify-self-center xl:col-span-1 xl:max-w-none">
                      <div class="grid grid-cols-[104px_minmax(140px,1fr)_104px] items-end gap-2">
                        <label class="block">
                          <span class="mb-1 block text-[11px] font-medium text-app-muted">下界（{{ item.unit }}）</span>
                          <input
                            class="field-input text-right text-xs font-semibold"
                            type="number"
                            ...
                            :step="item.step ?? 'any'"
                            aria-label="容量下界"
                            @change="updateCapacityBound(item, 'lowerBound', $event)"
                          >
                        </label>

                        <div class="min-w-0 pb-0.5">
                          <div class="mb-1 truncate text-center text-[11px] font-medium text-app-muted">
                            推荐值
                            <strong class="ml-1 text-xs text-primary">{{ formatNumber(item.suggestedValue) }} {{ item.unit }}</strong>
                          </div>
                          <input
                        ...
                        <label class="block">
                          <span class="mb-1 block text-[11px] font-medium text-app-muted">上界（{{ item.unit }}）</span>
                          <input
                            class="field-input text-right text-xs font-semibold"
                            type="number"
                            ...
                            :step="item.step ?? 'any'"
                            aria-label="容量上界"
                            @change="updateCapacityBound(item, 'upperBound', $event)"
                          >
                        </label>
                      </div>

                      <p v-if="variableError(item)" class="mt-1 text-right text-[11px] text-app-danger">{{ variableError(item) }}</p>
                    </div>

                    <div v-else class="col-span-3 flex h-10 w-full max-w-[520px] items-center justify-center justify-self-center rounded-md border border-dashed border-app-border bg-white/50 px-4 text-center text-xs text-app-muted xl:col-span-1 xl:max-w-none">
                      该设备不参与优化，将保持当前容量 {{ formatNumber(item.currentValue) }} {{ item.unit }}。
                    </div>
                  </article>
                </div>
                <div v-else class="px-6 py-12 text-center text-sm text-app-muted">当前画布没有可规划容量的设备。</div>

                <div class="border-t border-app-border bg-app-panel-soft px-5 py-2.5 text-right text-xs text-app-muted">
                  参与容量优化/可规划设备：<span class="font-bold text-primary">{{ optimizedCount }}/{{ variables.length }}</span>
                </div>
              </div>
            ...
            <section v-else-if="currentStep === 2" class="space-y-4">
              <div class="flex items-start justify-between gap-5 rounded-lg border border-primary/25 bg-blue-50 px-5 py-4">
                <div class="flex gap-3">
                  <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary text-lg text-white">🔒</span>
                  <div>
                    <div class="font-semibold text-app-text">历史边界数据由边界配置统一管理</div>
                    <p class="mt-1 text-xs leading-5 text-app-muted">
                      本页仅查看已保存的边界数据，所有字段均已锁定。点击表格中的“预览”可在下方查看对应时序曲线；若需修改，请返回边界配置。
                    </p>
                  </div>
                </div>
                <span class="shrink-0 rounded-full border border-primary/20 bg-white px-3 py-1 text-xs font-medium text-primary">只读</span>
              </div>
              ...
              <div class="rounded-lg border border-app-border bg-white">
                <div class="flex items-center justify-between border-b border-app-border px-5 py-4">
                  <div>
                    ...
                    <p class="mt-1 text-xs text-app-muted">直接读取项目中已配置的风速、辐照度、负荷等边界。</p>
                  </div>
                    ...
                    <tbody>
                      ...
                      >
                        ...
                        <td class="px-4 py-3 text-center">
                          ...
                          >
                            {{ selectedBoundaryId === boundary.id ? '预览中' : '预览' }}
                          </button>
                        </td>
                      </tr>
                    </tbody>
              ...
              <div class="rounded-lg border border-app-border bg-white">
                <div class="flex items-center justify-between border-b border-app-border px-5 py-4">
                  <div>
                    ...
                    <p class="mt-1 text-xs text-app-muted">选择上方边界后，在此查看边界配置中的时序数据曲线。</p>
                  </div>
...
.capacity-slider::-webkit-slider-runnable-track {
  height: 6px;
  border-radius: 999px;
  background: #c9d8ec;
}
...
.capacity-slider::-webkit-slider-thumb {
  ...
  filter: drop-shadow(0 2px 2px rgb(29 33 41 / 22%));
  cursor: grab;
  appearance: none;
}
...
.capacity-slider:focus-visible {
  box-shadow: 0 0 0 3px rgb(10 77 162 / 16%);
}

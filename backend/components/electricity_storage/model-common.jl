# ═══════════════════════════════════════════════════════════════════════════
# electricity_storage/model-common.jl — 共享函数
#
# model-base.jl 和 model.jl 共用的参数解析与辅助函数
# ═══════════════════════════════════════════════════════════════════════════

function resolve_electricity_storage_params(component::ElectricityStorage, ctx::BuildContext)
    ...
    return (
        ...
        # ramp 是充放电倍率（1/h）：倍率 × 能量容量 = 功率上限，不应再乘时段长度。
        ramp_ratio=component_paras(component)["ramp"],
        # 功率（kW）转换为单时段电量（kWh）时使用。
...
function derive_storage_state(planned_input::Vector{Float64}, planned_output::Vector{Float64})
    return [

...
function build_component_model!(model, component::WindTurbine, ctx::BuildContext, tracer::CodeTracer)
    params = resolve_wind_turbine_params(component, ctx)
    code = component_code(component)
    return build_wind_turbine_status_model!(model, component, params, ctx, code, tracer, Val(Symbol(params.status)))
end

function build_wind_turbine_status_model!(model, component::WindTurbine, params, ctx::BuildContext,
    ...
    # ── 表达式：可用功率（存入 model 供前端渲染）────────────────────────
    add_tracked_expression!(model, tracer, "AVAILABLE_WT_$(code)",
        "model[Symbol(\"AVAILABLE_WT_$(code)\")] = @expression(model, [t in $(format_val(time_index))], $(format_val(available_power))[t])",
        @expression(model, [t in time_index], available_power[t])
    )

    # ── 变量 ──────────────────────────────────────────────────────────
    E_WT = add_tracked_variable!(model, tracer, "E_WT_$(code)", time_index;
        lower_bound = 0.0, upper_bound = available_power)
    E_WT_cut = add_tracked_variable!(model, tracer, "E_WT_cut_$(code)", time_index;
        lower_bound = 0.0, upper_bound = available_power)
    ...
    # ── 约束：主动有功变化率 ──────────────────────────────────────────
    # P = P_available - P_cut，故只约束弃风功率的主动变化，不把自然可用
    # 功率变化误当作场站主动调节。
    if params.ramp_constraint_on && length(time_index) > 1
        ramp_time_index = time_index[1:(end-1)]
        add_tracked_linear_constraint!(model, tracer,
            "@constraint(model, [t in $(format_val(ramp_time_index))], E_WT_cut_$(code)[t] - E_WT_cut_$(code)[t+1] <= $(format_val(params.ramp_limit_kw)))",
            () -> @constraint(model, [t in ramp_time_index],
                E_WT_cut[t] - E_WT_cut[t+1] <= params.ramp_limit_kw)
        )
        add_tracked_linear_constraint!(model, tracer,
            "@constraint(model, [t in $(format_val(ramp_time_index))], E_WT_cut_$(code)[t+1] - E_WT_cut_$(code)[t] <= $(format_val(params.ramp_limit_kw)))",
            () -> @constraint(model, [t in ramp_time_index],
    ...
    if params.curtailment_constraint_on && params.max_cut_ratio < 1.0
        add_tracked_linear_constraint!(model, tracer,
            "@constraint(model, sum(E_WT_cut_$(code)) <= $(format_val(params.max_cut_ratio)) * sum($(format_val(available_power))))",
    ...
    if params.cut_objective_on
        cost_expr = sum(E_WT_cut) * params.cut_cost
        add_tracked_expression!(model, tracer, "C_wt_cut_$(code)",
            "@expression(model, C_wt_cut_$(code), sum(E_WT_cut_$(code)) * $(format_val(params.cut_cost)))",

...
function _get_algorithms(project_json::Dict)
    return get(project_json, "algorithms", Dict{String,Any}())
end

"""从 project.json 中获取节点列表"""
function _get_nodes(project_json::Dict)
    canvas = get(project_json, "canvas", Dict())
    return get(canvas, "nodes", [])
end
...
function _parse_component_nodes(nodes::Vector)
    ...
    for node in nodes
        ...
        eco_params = get(business, "economicParams", Dict())
        tech_params = get(business, "techParams", Dict())

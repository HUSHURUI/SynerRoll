...
#### 日前层 (Layer 1)
```
objective_value = 运维成本 + 启停成本 + 调整成本 + ...
```
...
struct CostItem
    ...
    cost_label::String         # 显示标签 (如 "运维成本", "启停成本", "松弛惩罚")
...
计算煤电启停成本。
- status_vector: 启停状态数组 (0或1)
- on_off_cost_rate: 单次启停成本 (元/次)
- 返回: 总启停成本 (元)
"""
...
| CP (煤电) | on_off_cost | 启停成本 | 启停次数 × 单次成本 | |
